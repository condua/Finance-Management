...
import TabButtons, {
  TabButtonType,
} from "@/src/components/navigation/TabButtons";
...
const Page = () => {
  ...
  const { categoryId, icon, name, type, transactionId } =
  ...
  const [createTransaction, createdTransactionResult] =
  ...
  const handleSubmit = async () => {
    ...
    if (activeBudgets) {
      const [isOverSpent, data] = isOverSpentBudget(
        activeBudgets,
        transaction.amount
      );
      if (isOverSpent) {
        Alert.alert(
          t("actions.warning"),
          t("actions.budgetwarning", {
            budgets: (data as string[]).join(", "),
          }),
          [
            {
            ...
            <Header
              ...
              headerRight={() => (
                <HeaderButton
                  ...
                  onPress={() =>
                    router.navigate(
                      "/(authenticated)/(tabs)/transaction/camera"
                    )
                  }
